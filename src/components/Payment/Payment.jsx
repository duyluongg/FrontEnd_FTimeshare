...
export default function Payment() {
    ...
    useEffect(() => {
        const getProductData = async () => {
            try {
                const productResponse = await axios.get(`https://bookinghomestayfpt.azurewebsites.net/api/products/viewById/${productID}`);
                setProductData(productResponse.data[0]);

                const updatedProjects = await (async () => {
                    const feedbackResponse = await axios.get(`https://bookinghomestayfpt.azurewebsites.net/api/feedback/average-feedback-rating/${productResponse.data[0].productID}`);
                ...
                const userResponse = await axios.get(`https://bookinghomestayfpt.azurewebsites.net/api/users/viewDetail/${productResponse.data[0].accID}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('https://bookinghomestayfpt.azurewebsites.net/api/bookings/customer/createbooking', formData, {
                headers: {
